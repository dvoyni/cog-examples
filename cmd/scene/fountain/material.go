...
// draws. The two tags are also what the camera's pass labels end in, which is
...
// the same value, so scene keys them to one material and batches equal tints,
// and the per-mote colour rides in Params instead: a colour inside the Material
// would make every mote a material of its own, fading every frame.
var sharedMote = scene.Material{Tags: m.NewList(scene.MaterialTag{
	Tag:    scene.TagForward,
	Shader: gfx.ShaderWithText(scenePrelude + moteShader),
	State:  twoSided(gfx.StateOpaque3D()),
})}

func moteMaterial() scene.Material { return sharedMote }
...
// occlude them.
func basinMaterial() scene.Material {
	return scene.Material{Tags: m.NewList(
		scene.MaterialTag{
			Tag:    tagGround,
