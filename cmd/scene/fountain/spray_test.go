...
// foxClips stands in for what the lookup's Clips reports of Fox.glb, so the
// gait is tested without an engine.
...
func TestAPassTagIsWhatTheLabelEndsIn(t *testing.T) {
	for label, want := range map[string]scene.PassTag{
		"scene.camera-100.ground":  tagGround,
		"scene.camera-100.forward": scene.TagForward,
		"canvas.layer":             "layer",
		"forward":                  scene.TagForward,
	} {
