...
// hud prints the residency table and the demo's own numbers, and clears the
// frame.
//
// Every number here is one the demo already knows: its clock, the Entities it
// spawned, the reloads its levers asked for, the residency the survey read and
// the instances the station table says a resident station contributes. None of
// it is read back out of the frame scene drew.
//
...
func hud(state *ecs.Write[*Residency], overlay *ecs.Write[*canvas.OpQueue]) {
	r, q := state.Get(), overlay.Get()
	r.rate.measure(time.Now())
	...
	text(q, hudLeft, hudTop, fmt.Sprintf(
		"loading  step %06d  time %.2fs  fps %.0f  %s   pose %s  morph %s",
		r.step, r.time(), r.rate.perSecond, settled,
		bytes(r.poseBytes), bytes(r.morphBytes)), hudColor)
	resident, instances := 0, len(stations)
	for i := range stations {
		if r.known[i] && r.states[i] == nil {
			resident++
			instances += stations[i].draws
		}
	}
	text(q, hudLeft, hudTop+hudLine, fmt.Sprintf(
		"entities %d  stations resident %d of %d  instances %d  reloads %d",
		r.entities, resident, len(stations), instances, r.reloads), hudDimColor)
	...
	for i := range stations {
		...
		text(q, x, y, r.stationLine(i), stateColor(r.states[i], &stations[i]))
	}

	text(q, hudLeft, screenHeight-hudFoot-hudLine,
		"u unload truck   t unload its textures   x unload all   r retry the failures",
		hudDimColor)
	text(q, hudLeft, screenHeight-hudFoot,
		"a bare pad is a Model that was skipped, never substituted", hudDimColor)
}

// stationLine is one station's row: its name, its residency, how many nodes its
// selector covered, and what it is here for.
func (r *Residency) stationLine(i int) string {
	station := &stations[i]
	nodes := "  -"
	if r.nodeCount[i] >= 0 {
		nodes = fmt.Sprintf("%3d", r.nodeCount[i])
	}
	return fmt.Sprintf("%-*s %-8s n%s  %s",
		hudNamePad, station.name, stateName(r.states[i]), nodes, station.note)
}
