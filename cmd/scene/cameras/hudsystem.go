...
type (
	...
	materialCount struct{ Material scene.Material }
)

// census is what the HUD counts of the world: every number in it is one the
// app knows because it made the Components, never one read back out of the
// frame scene drew.
type census struct {
	cameras, passes, depthTagged int
}

// hud composites both panels and draws everything over them.
func hud(
	cameras *ecs.Query[cameraCount],
	materials *ecs.Query[materialCount],
	state *ecs.Read[*State],
	overlay *ecs.Write[*canvas.OpQueue],
) {
	s, q := state.Get(), overlay.Get()
	if q == nil {
		return
	}
	var counted census
	for _, it := range cameras.All() {
		counted.cameras++
		// An empty Passes is one default pass, so a camera always costs one.
		counted.passes += max(it.Camera.Passes.Len(), 1)
	}
	for _, it := range materials.All() {
		for _, tag := range it.Material.Tags.All() {
			if tag.Tag == TagDepth {
				counted.depthTagged++
			}
		}
	...
	composite(q, layerViews, mainPanel, s.mainTexture, panelTint(s, views[0].view))
	composite(q, layerViews, mapPanel, s.mapTexture, panelTint(s, views[1].view))

	for _, panel := range views {
		nameplates(s, q, panel)
	}
	lines(s, q, counted)
}

// panelTint dims the panel the pointer is not over.
func panelTint(s *State, view panel) m.Color {
	if _, over := view.texel(s.pointer); over {
...
// half of this demo.
//
// Three rules, all of them the helpers' rather than this function's:
//
...
//     caller names the size it means - to scene.ViewProjection, whose aspect
//     is that size's, and to m.WorldToScreen - and the answer comes back in
//     that target's texels. Mapping those texels onto the screen is the
//     panel's job, and the minimap's is not the identity.
...
func nameplates(s *State, q *canvas.OpQueue, panel composited) {
	picked := s.pickedName()
	for i := range cubes {
		texel, ok := panel.worldToScreen(nameplateAnchor(i))
		if !ok || !panel.view.contains(texel) {
			continue
		}
		at := panel.view.canvas(texel)
		color := plateColor
		if picked == cubes[i].name {
...
// lines prints the demo's key numbers.
func lines(s *State, q *canvas.OpQueue, counted census) {
	running := "running"
	if s.paused {
		running = "paused"
	}
	...
	top := [...]string{
		fmt.Sprintf("cameras  step %06d  z %+.2f  fps %.0f  %s",
			s.step, trackZ(s.time()), s.rate.perSecond, running),
		fmt.Sprintf("spawned %d  cameras %d  passes %d  model %s",
			s.spawned, counted.cameras, counted.passes, resident),
		fmt.Sprintf("materials with a depth tag %d  picked %s",
			counted.depthTagged, picked),
	}
	for i, line := range top {
		text(q, hudLeft, hudTop+float32(i)*hudLine, line, hudColor)
	}
	...
	for i, line := range side {
		text(q, mapPanel.rect.X, column+float32(i)*hudLine, line, hudDimColor)
	}

	if reports, last := s.reports.read(); reports > 0 {
		text(q, mapPanel.rect.X, column+float32(len(side)+1)*hudLine,
			fmt.Sprintf("reports %d", reports), hudWarnColor)
		text(q, mapPanel.rect.X, column+float32(len(side)+2)*hudLine,
			truncate(last, 46), hudWarnColor)
	}

	text(q, hudLeft, screenHeight-hudFoot,
...
func text(q *canvas.OpQueue, x, y float32, line string, color m.Color) {
