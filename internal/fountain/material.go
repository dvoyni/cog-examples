...
// Vertex is the fountains' own layout, the procedural example's approach: a
// custom layout needs a custom material, and both are this file.
type Vertex struct {
	Position m.Vec3
	Normal   m.Vec3
}
...
// The camera's two pass tags. The ground pass draws the basin alone, beneath
// everything the forward pass draws, because only the basin's material has an
// entry for it. They are also the tags a pass's label ends in, which is how a
// fountain's HUD finds its camera's passes in a frame snapshot.
const (
	TagGround  = "ground"
	TagForward = "forward"
)
...
// Every mote shares one material, and its own colour rides in a draw
// parameter instead: a colour inside the material would make every mote a
// material of its own, fading every frame.
const MoteTintParam = "moteTint"

// The mote material, forward only.
func MoteShader() gfx.ShaderDescr  { return gfx.ShaderWithText(scenePrelude + moteShader) }
func MoteState() gfx.MaterialState { return twoSided(gfx.StateOpaque3D()) }

// The basin material serves both pass tags: lit stone in the ground pass, and
// ripples blended over it in the forward pass, where the motes and the fox
// occlude them.
func StoneShader() gfx.ShaderDescr   { return gfx.ShaderWithText(scenePrelude + stoneShader) }
func StoneState() gfx.MaterialState  { return twoSided(gfx.StateOpaque3D()) }
func RippleShader() gfx.ShaderDescr  { return gfx.ShaderWithText(scenePrelude + rippleShader) }
func RippleState() gfx.MaterialState { return twoSided(gfx.StateTransparent3D()) }
