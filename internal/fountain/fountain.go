...
// FoxGait is one step of the fox's clips at time t: each clip's time advances
// at the rate that matches its stride to the fox's ground speed, which only the
// game knows, and the two weights follow the blend.
func FoxGait(t float32, walkTime, runTime *float32) (walk, run float32) {
	speed := FoxSpeed(t)
	*walkTime += FixedStep * speed / WalkPace
	*runTime += FixedStep * speed / RunPace
	return FoxBlend(speed)
}
